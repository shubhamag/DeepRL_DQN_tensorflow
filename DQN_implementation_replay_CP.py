...
#            target_batch = np.zeros((self.batch_size, self.numActions))
#            for i in range(self.batch_size):
#                s, a, r, s1, done = minibatch[i]
#                target = target_model.predict(s[None])
#                if not done:
#                    target[0][a]= r + self.gamma*np.amax(target_model.predict(s1[None]))
#                else:
#                    target[0][a]= r
#                state_batch[i, :] = s
#                target_batch[i, :] = target[0]
#            self.model.fit(state_batch, target_batch, verbose=0) 
...
#                print("annealed eps to ", self.eps)
            ...
            if (is_done == True):
                ...
                if numEpisodes % 500 == 1:
                    ...
                    self.QNet.save_model("model_mlp_small_cartpole.json")
                    self.QNet.save_model_weights("model_mlp_DQN_cartpole.h5")
...
#                target_model.set_weights(self.model.get_weights())
            ...
            if (done ==True):
                ...
                if(done_count==20):
                    ...
                    if (abs(avg_reward) > abs(self.max_test_reward)):
                        ...
                        self.QNet.save_model_weights("best_weights_DQN_CP.h5")
                    reward_count=0
                    done_count=0
                    break
        return
    def write_test_results(self, filename="DQN_CP_test_results"):
    ...
    def wrap_and_record(self,paths):
        self.env = gym.wrappers.Monitor(self.env, './dqn_cp',
...
def main(args):    
    ...
    agent.train()
    # agent.wrap_and_record(["best_weights_DQN_CP.h5"])
