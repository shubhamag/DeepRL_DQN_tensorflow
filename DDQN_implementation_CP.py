...
class DQN_Agent():
    ...
    def __init__(self, environment_name="CartPole-v0", render=False):
        ...
        self.model_weights_file = "model_DDQN_CP_fri_"
        ...
        self.replay = Replay_Memory(burn_in=1000)
        self.burn_in_memory()
...
#                target_model.set_weights(self.model.get_weights())
            ...
            if (done == True):
                ...
                if (done_count == 20):
                    ...
                    if (abs(avg_reward) < abs(self.max_test_reward)):
                        ...
                        self.QNet.save_model_weights("best_weights_DDQN_CP.h5")
                    reward_count = 0
                    done_count = 0
    ...
    def wrap_and_record(self,paths):
        self.env = gym.wrappers.Monitor(self.env, 'cartpole-linear-replay/cart_pole_linear',
    ...
    def write_test_results(self, filename="DDQN_replay_CP_test_results"):
        file = open(filename, 'w')
        for item in self.test_results:
            file.write("%s\n" % item)

        file.close()
        file = open("DDQN_replay_CP_train_results", 'w' )
...
def main(args):    
    ...
    # agent.train()
    agent.test_and_log("model_DDQN_CP_fri_1251.h5")
    agent.wrap_and_record(["model_DDQN_CP_fri_251.h5","model_DDQN_CP_fri_251.h5","model_DDQN_CP_fri_751.h5","model_DDQN_CP_fri_1251.h5"])
    # agent.test_record_video("model_DDQN_CP_fri_251.h5")
    # agent.test_record_video("model_DDQN_CP_fri_751.h5")
    # agent.test_record_video("model_DDQN_CP_fri_1251.h5")
